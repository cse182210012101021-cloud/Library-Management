...
export class DashboardService {
    static async getDashboardData(userId: string | null, userType: string | null) {
        ...
        if (userType === UserType.STUDENT && userId) {
            ...
            totalApplications = await Application.countDocuments({ appliedBy: userId });
            pendingApplications = await Application.countDocuments({ appliedBy: userId, status: ApplicationStatus.PENDING });
        } else {
            // Fetch admin view
        ...
        // 3. Recent Books
        const recentBooks = await Book.find().sort({ createdAt: -1 }).limit(5).select("title author coverImage _id");
