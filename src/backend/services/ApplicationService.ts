  ...
  static async requestReturn(applicationId: string) {
    ...
    for (const adminId of adminIds) {
      await NotificationService.createNotification({
        ...
        message: `A student has requested to return a book (ID: ${(application as any)._id.toString().slice(-6).toUpperCase()}).`,
        type: NotificationType.INFO,
        link: "/applications",
      });
    }
  ...
  static async updateApplication({
    ...
    for (const adminId of adminIds) {
      await NotificationService.createNotification({
        ...
        message: `A student has updated their pending book application (ID: ${(application as any)._id.toString().slice(-6).toUpperCase()}).`,
        type: NotificationType.INFO,
        link: "/applications",
      });
    }
